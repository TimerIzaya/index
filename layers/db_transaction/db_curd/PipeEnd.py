...
from layers.db_transaction.db_curd.ValueGenerator import ValueGenerator
...
class PipeEnd:
    ...
    def generate_il(self, store_id: Identifier, key, irctx, idbctx):
        """
        ...
        for param in self.method_info.params:
            ...
            if param.enum:
                ...
                index_name = idbctx.pick_random_index()
                ...
                ident = irctx.get_random_identifier(IDBType.IDBRequest.value)
                args.append(ident or Literal("<request>"))
            else:
                # 使用统一生成器处理剩余所有普通类型
                args.append(Literal(valgen.generate(typename, key=key)))

        result_name = irctx.generate_unique_name(f"req_{self.name}")
        ...
        irctx.register_variable(Variable(name=result_name, type_=Type(return_typename)))
        return call
